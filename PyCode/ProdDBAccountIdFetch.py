...
def tier_ids_fetch(env, brand_name):

    team_config, main_config = get_team_secret(env)

    TEAM_DB_CONFIG = {
        "host": team_config["host"],
        "user": team_config["username"],
        "password": team_config["password"],
        "database": team_config["dbname"]
    }
    ...
    MAIN_DB_CONFIG = {
        ...
        "database": main_config["dbname"]
    }

    # print(TEAM_DB_CONFIG)


    tier_1_id = None
    tier_2_id = None

    team_db_conn = mysql.connector.connect(**TEAM_DB_CONFIG)
    team_db_cursor = team_db_conn.cursor(dictionary=True)

    main_db_conn = mysql.connector.connect(**MAIN_DB_CONFIG)
    main_db_cursor = main_db_conn.cursor(dictionary=True)
    ...
    try:
        ...
        main_db_cursor.execute(
            ...
            select id
            from brand
            where name = %s and inactive = 0
            """
        , (brand_name,))

        brand = main_db_cursor.fetchone()
        main_db_conn.commit()

        if not brand:
            raise Exception('No brand found!')

        brand_id = brand["id"]

        team_db_cursor.execute(
            """
            select id 
            from tiers 
        ...
        parent = team_db_cursor.fetchone()
        team_db_conn.commit()

        if not parent:
            raise Exception(f"{brand_name} not found/ {brand_name} with tier 1 not found")

        parent_id = tier_1_id = parent["id"]
        ...
        tier_2 = team_db_cursor.fetchone()
        team_db_conn.commit()

        if not tier_2:
            raise Exception(f"{brand_name} with tier 2 not found")

        print("Tier 2 ID: ", tier_2["id"])
        tier_2_id = tier_2["id"]

...
def getResourceGroup(subs_id):
    ...
    for rg in resource_client.resource_groups.list():
        if "ResourceGroup" in rg.name and "ResourceGroupEU" not in rg.name:
...
def getResourceJSON(subscription_id):
    ...
    # brand_name = scope_name.replace(" ", "")
    # brand_name = "WorkingSolutions"
    resource_group_name = getResourceGroup(subscription_id)
    ...
    for service, resource_name in resources.items():
        try:
            ...
            returnJson[service] = [resource.name, keys.key1, endpoint]
    ...
    for resource_json in resource_data_list:
        for key, value in resource_json.items():
            service_name = resource_json[key]["resource_type"].replace(' ', '').lower()
            returnJson[service_name] = resource_json
        print(resource_json)
...
# def main():
#
#     subscription_id = "0498000f-30d8-4f5d-8a77-7d9afc9c3f49"
#     # credential = DefaultAzureCredential()

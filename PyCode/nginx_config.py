...
def get_template_site_name(env, site_name):
    ...
    elif env == "qa.zenarate.com":
        return "qa.zenarate.com", f"qa-{site_name}"
    elif env == "qa":
        return "qa.zenarate.com", f"qa-{site_name}"
    elif env == "qa2":
        return "qa2.zenarate.com", f"qa2-{site_name}"
    elif env == "perf.zenarate.com":
        return "perf.zenarate.com", f"perf-{site_name}"
    else:
        raise Exception(f"Unknown environment: {env}")

def env_tag_mapper(env):

    if env == "prod":
        return "ServerType", "BuildServer"
    elif env == "beta":
        return "env", "zenarate/beta"
    elif env == "qa2":
        return "Name", "qa2"
    elif env == "qa":
        return "Name", "qa.zenarate.com"
    elif env == "perf":
        return "Name", "perf.zenarate.com"
...
def get_instances_by_tag_substr(tag_key, tag_value_substring, excluded_name_list):
    """
    Fetch all running EC2 instances where the tag value contains a specific substring.
    :param tag_key: The key of the tag (e.g., 'env').
    :param tag_value_substring: A substring to search for in the tag's value (e.g., 'qa', 'perf').
    :param excluded_name_list: List of instance 'Name' tags to exclude (case-insensitive match).
    :return: List of [instance_id, name_tag_value].
    """
    response = ec2_client.describe_instances(
        Filters=[
    ...
    for reservation in response['Reservations']:
        for instance in reservation['Instances']:
            ...
            # Find the value of the target tag_key (e.g., 'env')
            target_tag_value = next(
                (tag['Value'] for tag in tags if tag['Key'].lower() == tag_key.lower()),
                None
            )

            # Check if the tag value contains the specified substring (case-insensitive)
            if not target_tag_value or tag_value_substring.lower() not in target_tag_value.lower():
                continue

            # Get the Name tag if it exists
            name_tag_value = next(
                (tag['Value'] for tag in tags if tag['Key'].lower() == 'name'),
                None
            )

            # Exclude instance if its Name tag matches anything in the excluded list
            if name_tag_value and specific_names_to_be_excluded(excluded_name_list, name_tag_value):
                continue

            instances.append([instance['InstanceId'], name_tag_value])
...
def main():
    ...
    parser.add_argument('--exectype', required=True, help='brand url')
    ...
    tag_key, tag_value = env_tag_mapper(env)

    # tag_key = "ServerType" # Checked/Changed by the programmer
    # tag_value = "BuildServer" # Checked/Changed by the programmer

    site_name = url  # Changed by the programmer
    ...
    instances = get_instances_by_tag_substr(tag_key, tag_value, excluded_name_list)
    ...
    if exectype == "read":
        command_to_execute = commands_read
    elif exectype == "write":
        command_to_execute = commands

    if command_to_execute != "":
        if instances:
            command_id = execute_commands(instances_ids, command_to_execute)
